...
from Variables import path_damaged, path_undamaged, BATCH_SIZE, rep_dim, lr_pretrain, epochs_pretrain, weight_decay_pretrain, lr, epochs, weight_decay, num_class, eps, nu, rep_dims, data_saved
...
for rep_dim in rep_dims:
    SVDD_trainer = train_model(lr_pretrain, weight_decay_pretrain, epochs_pretrain, lr, weight_decay, epochs, device, train_loader, rep_dim, num_class, eps, nu)
    net, R, c = SVDD_trainer.train()

    SVDD_tester = test_model(net, R, c, test_loader, device)
    SVDD_tester.confusion_mat(rep_dim)
