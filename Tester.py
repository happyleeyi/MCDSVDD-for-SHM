...
class test_model:
    def __init__(self, net, R, c,  train_loader, test_loader, device):
        self.net = net
        self.R = R
    ...
    def test(self):
        ...
        score_idx = [score[i]>self.R for i in range(score.shape[0])]
    ...
    def test2(self, rep_dim, bandwidth):
        predict_test = []
        truevalue_test = []
        score = []
        score = np.array(score)
    ...
    def confusion_mat(self, rep_dim, nu, bandwidth, use_kde):
        if use_kde:
            predict_test, truevalue_test = self.test2(rep_dim, bandwidth)
        else:
